...
def reset_dashboard_data(admin_id):
    ...
    if not super_admin or super_admin.id != admin_id:
        LevelSensorData.__table__.drop(db.engine)  # Drop existing table if exists
        create_sensor_data_table(f'level_sensor_data_{admin_name}')  # Create new table using admin's name
        db.session.commit()
...
def admin_dashboard(adminname):
    if 'email' in session:
        ...
        LevelSensorData.__table__.name = table_name
...
def add_sensor_data(adminname):
    ...
    table_name = f'level_sensor_data_{adminname}'
    LevelSensorData.__table__.name = table_name  # Point the model to the correct table
    ...
    new_data = LevelSensorData(
        date=parsed_data.get('D'),
        full_addr=parsed_data.get('address'),
        sensor_data=parsed_data.get('data')[0],  # Assuming the data is a list and you want the first element
        vehicleno=parsed_data.get('Vehicle no'),
        volume_liters=None  # Assuming this field is optional and can be null
    )
